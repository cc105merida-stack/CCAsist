...
async def guardar_siac_folio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not worksheet_data:
        ...
        return ConversationHandler.END
    try:
        worksheet_data.update_cell(cliente['fila'], 4, folio)  # Columna D (folio)
        nuevos_resultados = [r for r in resultados if r['fila'] != cliente['fila']]
        busqueda_siac[telegram_id]['resultados'] = nuevos_resultados
        busqueda_siac[telegram_id]['pagina'] = pagina
