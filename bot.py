...
# ========== CONFIGURACIÓN ==========
# TOKEN DE TELEGRAM - REEMPLAZA CON EL TUYO
TOKEN = "8638219086:AAFXkpgEYQbGTm9nhODPXxaT5jmrLMEbgPc"

# ID DE GOOGLE SHEETS - REEMPLAZA CON EL TUYO
SPREADSHEET_ID = "16CXC3NDTCUf-TkUKroLOSXPYSqiS2YQQzVfhT_R5lfQ"
...
class LlamadaEstado:
    def __init__(self):
        self.usuarios_activos = {}  # {telegram_id: info_llamada}
        self.pendientes_notificacion = {}  # {telegram_id: info_llamada_pendiente}
        self.recontactos_pendientes = {}  # {telegram_id: info_llamada_recontacto}

llamadas_activas = LlamadaEstado()

# Diccionario para almacenar imágenes para PDF por usuario
imagenes_para_pdf = {}  # {telegram_id: lista_de_rutas_de_imagenes}
...
def obtener_agente_por_nombre(worksheet_registros, nombre_agente):
    ...
    try:
        ...
        print(f"Error al obtener agente: {e}")
        return None
...
def obtener_llamadas_pendientes(data_worksheet):
    ...
    try:
        ...
        for i, fila in enumerate(todos_los_registros[1:], start=2):
            if len(fila) >= 20:
                # ESTADO2 está en la columna S (índice 18)
                estado2 = fila[18] if len(fila) > 18 else ""
                # AGENTE_ASIGNADO está en la columna T (índice 19)
        ...
        print(f"Error al obtener llamadas pendientes: {e}")
        return []
...
def obtener_siguiente_llamada_disponible(data_worksheet):
    ...
    try:
        ...
        print(f"Error al obtener siguiente llamada: {e}")
        return None, None
...
def obtener_hoja_registros(sheet):
    ...
    try:
        ...
        print(f"Error al obtener hoja de registros: {e}")
        return None
...
def obtener_hoja_data(sheet):
    ...
    try:
        ...
        print(f"Error al obtener hoja Data: {e}")
        return None
...
def actualizar_estado_llamada(data_worksheet, fila_numero, nuevo_estado):
    ...
    try:
        ...
        print(f"✅ Estado actualizado: fila {fila_numero} -> {nuevo_estado}")
        return True
    except Exception as e:
        print(f"❌ Error al actualizar estado de llamada: {e}")
        return False
...
def registrar_hora_asignacion(data_worksheet, fila_numero):
    ...
    try:
        ...
        print(f"✅ Hora registrada: fila {fila_numero} -> {hora_actual}")
        return hora_actual
    except Exception as e:
        print(f"❌ Error al registrar hora de asignación: {e}")
        return None
...
def registrar_duracion_y_observacion(data_worksheet, fila_numero, resultado, observacion="", hora_asignacion=None, estado_validacion=None):
    ...
    try:
        ...
        print(f"Error al registrar duración y observación: {e}")
        return False
...
async def iniciar_creacion_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Inicializar lista de imágenes para este usuario
...
async def recibir_imagen_para_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Verificar si el usuario está en modo creación de PDF
...
async def generar_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Verificar si el usuario tiene imágenes guardadas
...
async def obtener_nombre(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        print(f"Error en obtener_nombre: {e}")
        await update.message.reply_text(
...
async def obtener_llamada(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if actualizar_estado_llamada(worksheet_data, fila_num, "EN PROCESO"):
        ...
        try:
            ...
            print(f"✅ Validador actualizado: {nombre_usuario} en fila {fila_num}")
        except Exception as e:
            print(f"❌ Error al actualizar validador: {e}")
    else:
        await update.message.reply_text(
    ...
    if 'pendientes_notificacion' not in context.bot_data:
        ...
        for clave in claves_anteriores:
            if clave not in claves_actuales:
                ...
                print(f"✅ Eliminada clave antigua: {clave}")
    ...
    try:
        ...
        if worksheet_data:
            try:
                ...
                print(f"✅ Agente asignado guardado: {llamada['validador']} en fila {llamada['fila']}")
            except Exception as e:
                print(f"Error al guardar agente asignado: {e}")
        ...
        if 'pendientes_notificacion' in context.bot_data and clave in context.bot_data['pendientes_notificacion']:
            del context.bot_data['pendientes_notificacion'][clave]
            print(f"✅ Llamada {clave} eliminada de pendientes_notificacion")
    ...
    try:
        ...
        if actualizar_estado_llamada(worksheet_data, fila_num, "EN PROCESO"):
            try:
                ...
                print(f"✅ Agente asignado limpiado en fila {fila_num}")
            except Exception as e:
                print(f"Error al limpiar agente asignado: {e}")
            
            # Limpiar de la lista del supervisor
            ...
            try:
                ...
                print(f"✅ Validador actualizado: {nombre_agente} en fila {fila_num}")
            except Exception as e:
                print(f"Error al actualizar validador: {e}")
            
            if llamada_id in context.bot_data['recontactos_pendientes']:
                del context.bot_data['recontactos_pendientes'][llamada_id]
                print(f"✅ Llamada {llamada_id} eliminada de recontactos_pendientes")
...
async def manejar_resultado_llamada(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Guardar datos temporales para todos los casos
    ...
    # Si el resultado es "completada", mostrar menú de validación
    if resultado == "completada":
        # Mostrar menú de validación
        keyboard = [
            [
    ...
    # Para todos los demás resultados (no contesta, rechazada), solicitar observación directamente
    await query.edit_message_text(
...
async def manejar_validacion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Mapear opciones a valores con capitalización correcta
    opciones_validacion = {
        "validada": "Validada",
...
async def recibir_observacion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Si hay una conversación de registro activa, salir
...
def main():
    ...
    print("🤖 Bot iniciado...")
    print(f"📊 Usando Google Sheets ID: {SPREADSHEET_ID}")
